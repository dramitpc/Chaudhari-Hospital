...
function commonDefinition(content: Content[], title: string, pageSize: "A4" | "A5" | "LETTER", baseSize: number): TDocumentDefinitions {
  return {
    info: { title, creator: "ClinicOS" },
    pageSize,
    pageMargins: [40, 36, 40, 42],
    defaultStyle: { font: "NotoSans", fontSize: baseSize, color: "#0f172a", lineHeight: 1.15 },
    styles: {
      clinic: { fontSize: 18, bold: true, color: BRAND },
      doctor: { fontSize: 11, bold: true, alignment: "right" },
      section: { fontSize: 7.5, bold: true, color: MUTED, characterSpacing: 0.5, margin: [0, 0, 0, 3] },
    },
    content,
    footer: (current, total) => ({
      columns: [{ text: "Generated by ClinicOS", color: MUTED }, { text: `Page ${current} of ${total}`, alignment: "right", color: MUTED }],
      font: "NotoSans", fontSize: 7, margin: [40, 8, 40, 0],
    }),
  };
}

function letterhead(settings: ClinicSettings | undefined, doctorName: string | null | undefined, specialization?: string | null, align: "left" | "center" = "center"): Content {
  const clinicStack: Content[] = [
    ...
    ...(settings?.registrationNumber ? [{ text: `Reg: ${settings.registrationNumber}`, color: MUTED, fontSize: 7, alignment: align } as Content] : []),
  ];
  return {
    table: {
      widths: ["*", "*"],
      body: [[
        { stack: clinicStack, border: [false, false, false, true], borderColor: [BRAND, BRAND, BRAND, BRAND], margin: [0, 0, 0, 8] },
        { stack: [{ text: doctorName ?? "", style: "doctor" }, ...(specialization ? [{ text: specialization, color: MUTED, alignment: "right" } as Content] : [])], border: [false, false, false, true], borderColor: [BRAND, BRAND, BRAND, BRAND], margin: [0, 0, 0, 8] },
      ]],
    },
...
function section(label: string | Content, value: string | null | undefined, font = "NotoSans", color?: string): Content[] {
  if (!value?.trim()) return [];
  return [
    { text: label, style: "section", margin: [0, 0, 0, 2] },
    { text: value, font, color, margin: [0, 0, 0, 10], preserveLeadingSpaces: true },
  ];
}

function prescriptionItems(input: PrescriptionPdfInput, translatedFont: string): Content {
  ...
  const cell = (english: string | null | undefined, translatedText: string | null | undefined): Content => {
    if (bilingual && translatedText && translatedText !== english) {
      return { stack: [{ text: english || "—" }, { text: translatedText, font: translatedFont, color: TRANSLATED, margin: [0, 2, 0, 0] }] };
    }
  ...
  if (format.drugStyle === "list") {
    return {
      ol: original.map((item, index) => {
        ...
        const lines: Content[] = [
          { text: [{ text: item.drugName, bold: true }, ...(format.showGenericName && item.genericName ? [{ text: ` (${item.genericName})`, color: MUTED, fontSize: 7 }] : [])] },
        ...
        return { stack: lines, margin: [0, 0, 0, 6] };
      }),
      margin: [12, 0, 0, 10],
    };
  }
  ...
  return {
    table: {
      ...
      body: [headers.map(value => ({ text: value, bold: true, fontSize: 7.5, fillColor: LIGHT })), ...rows],
      dontBreakRows: true,
    },
    layout: { hLineColor: () => BORDER, vLineColor: () => BORDER, paddingLeft: () => 6, paddingRight: () => 6, paddingTop: () => 5, paddingBottom: () => 5 },
    margin: [0, 0, 0, 12],
  };
}
...
export async function createPrescriptionPdf(input: PrescriptionPdfInput): Promise<Blob> {
  ...
  const content: Content[] = [
    letterhead(settings, prescription.doctorName, prescription.doctorSpecialization, format.headerAlign),
    ...(prescription.doctorConsultingHours ? [{ text: [{ text: "Consulting Hours: ", bold: true }, prescription.doctorConsultingHours], alignment: "center", color: MUTED, margin: [0, -8, 0, 8] } as Content] : []),
    {
      table: {
        ...
        ].map(text => ({ text, fillColor: LIGHT, margin: [5, 5, 5, 5] }))],
      },
      layout: { hLineColor: () => BORDER, vLineColor: () => BORDER },
      margin: [0, 0, 0, 3],
    },
    { text: `${fmtDate(prescription.visitDate)}`, alignment: "right", color: MUTED, margin: [0, 0, 0, 10] },
  ];

  if (patient?.allergies) {
    content.push({ table: { widths: ["auto", "*"], body: [[{ text: "ALLERGIES:", bold: true, color: "#991b1b" }, { text: patient.allergies, color: "#991b1b" }]] }, layout: "noBorders", fillColor: "#fef2f2", margin: [0, 0, 0, 10] });
  }
  if (format.showSoap) {
    const soap = [
    ...
    content.push(...section("SOAP NOTES", soap));
  }
  ...
  if (format.showDiagnosis) content.push(...section("DIAGNOSIS", prescription.diagnosis));
  content.push({ text: textLabel("medications", language, bilingual), style: "section" }, prescriptionItems(input, translatedFont));

  const orthotics = (() => { try { const parsed = JSON.parse(prescription.notes ?? "[]"); return Array.isArray(parsed) ? parsed.join("   •   ") : ""; } catch { return ""; } })();
  content.push(...section("ORTHOTICS", orthotics));
  if (format.showAdvice) {
    if (bilingual) {
      content.push(...section(textLabel("advice", language), prescription.advice));
      content.push(...section("", translation?.advice, translatedFont, TRANSLATED));
    } else {
      content.push(...section(textLabel("advice", language, !showTranslated), showTranslated ? (translation?.advice ?? prescription.advice) : prescription.advice, showTranslated ? translatedFont : "NotoSans"));
    }
  }
  if (format.showFollowUp && prescription.followUpDate) content.push(...section("FOLLOW-UP", fmtDate(prescription.followUpDate)));
  if (format.showReferenceTo) content.push(...section("REFERENCE TO", prescription.referenceTo));

  const signature: Content[] = [];
  if (prescription.doctorSignatureData?.startsWith("data:image/")) signature.push({ image: prescription.doctorSignatureData, width: 100, alignment: "right" });
  signature.push({ text: prescription.doctorName ?? "Doctor", bold: true, alignment: "right" });
  if (prescription.doctorRegistrationNumber) signature.push({ text: `Reg. No: ${prescription.doctorRegistrationNumber}`, color: MUTED, fontSize: 7, alignment: "right" });
  signature.push({ text: "Signature & Stamp", color: MUTED, fontSize: 7, alignment: "right" });
  content.push({ stack: signature, margin: [0, 24, 0, 0] });

  const pageSize = format.paperSize === "a5" ? "A5" : format.paperSize === "letter" ? "LETTER" : "A4";
  const size = format.fontSize === "lg" ? 11 : format.fontSize === "md" ? 10 : 9;
  return render(commonDefinition(content, `Prescription - ${prescription.patientName}`, pageSize, size));
}
...
export async function createReceiptPdf({ invoice, patient, payments = [], settings, language }: ReceiptPdfInput): Promise<Blob> {
  ...
  return render(commonDefinition(content, `${isReceipt ? "Receipt" : "Invoice"} - ${invoice.invoiceNumber}`, "A4", 9));
}
