            ...
            {isLoading ? Array.from({ length: 4 }).map((_, i) => (
              ...
              <tr key={u.id} className="border-b border-border hover:bg-muted/20">
                <td className="px-4 py-3 font-medium">
                  {u.fullName}
                  {u.specialization && <span className="block text-xs text-muted-foreground">{u.specialization}</span>}
                </td>
                <td className="px-4 py-3 font-mono text-xs">{u.username}</td>
                <td className="px-4 py-3">
                  <span className={`inline-flex px-2 py-0.5 rounded text-xs font-medium ${roleColors[u.role] ?? ""}`}>{u.role}</span>
                </td>
